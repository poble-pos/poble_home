...
export const PricingSection: React.FC = () => {
  ...
  const description =
    config.description ||
    "Everything your venue needs for one transparent price.";

  return (
    <section
      id="pricing"
      className="py-24 relative overflow-hidden bg-white border-b border-slate-100 font-inter"
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-wrap justify-center gap-8 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200">
          {plans.map((plan: any, pIdx: number) => (
            <div
              key={plan.id}
              className={`relative p-8 sm:p-10 md:p-14 rounded-[3rem] md:rounded-[4rem] border-2 transition-all duration-700 flex flex-col w-full max-w-2xl bg-white border-poble-gold shadow-2xl z-10 group hover:-translate-y-2`}
            >
              ...
              <div className="flex flex-col md:flex-row gap-8 mb-10">
                <div className="flex-1 text-center md:text-left">
                  <h3 className="text-3xl font-black mb-3 tracking-tight text-poble-charcoal font-heading">
                    {plan.title}
                  </h3>
                  ...
                  <div className="mt-8">
                    <div className="flex items-baseline justify-center md:justify-start gap-2">
                      <span className="text-3xl font-black opacity-40 text-poble-charcoal">
                        from
                      </span>
                      <span className="text-7xl md:text-8xl font-black tracking-tighter text-poble-charcoal">
                        {plan.price}
                      </span>
                      <span className="font-black uppercase text-[11px] tracking-[0.2em] ml-2 text-slate-400">
                        / {plan.period || "month"}
                      </span>
                    </div>
                  </div>
                ...
                <div className="w-full md:w-48 h-48 rounded-3xl overflow-hidden bg-white border border-slate-100 shrink-0 shadow-inner group-hover:shadow-md transition-all">
                  <Image
                    src={
                      (pIdx === 0 ? config.item1_image : config.item2_image) ||
                      "/poble_logo.png"
                    }
                    alt={plan.title}
                    width={192}
              ...
              <ul className="space-y-6 mb-16 flex-grow">
                {(plan.features || defaultFeatures).map(
                  (feature: any, fIdx: number) => (
                    ...
                    >
                      <div className="mt-1.5 p-1 rounded-full bg-slate-50 group-hover/item:bg-poble-gold/10 transition-colors">
                        <Check
                          className="w-3.5 h-3.5 text-poble-charcoal group-hover/item:text-poble-gold"
                          strokeWidth={4}
                        />
                      </div>
                      <span
                        className={`text-lg font-bold tracking-tight text-poble-charcoal ${feature.included === false ? "opacity-30" : ""}`}
                      >
                        {feature.text}
                      </span>
                    </li>
        ...
        <div className="mt-20 text-center">
          <p className="text-sm font-bold text-slate-400 tracking-tight">
            *Hardware sold separately. Compatible with standard thermal
            printers. <br className="hidden md:block" />
