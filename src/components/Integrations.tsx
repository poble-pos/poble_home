...
const Column = ({
  ...
  <div className="relative h-[320px] md:h-[480px] overflow-hidden group/col px-1 md:px-2">
    <div
      className={`flex flex-col shrink-0 will-change-transform ${direction === "up" ? "animate-scroll-up" : "animate-scroll-down"}`}
    >
      {[
        ...items,
        ...items,
        ...
        <div
          key={i}
          className={`h-16 w-20 md:h-24 md:w-32 rounded-xl md:rounded-2xl border shadow-sm flex items-center justify-center transition-all duration-300 mb-4 md:mb-6 group ${
            p.name === "Tyro"
...
export const Integrations: React.FC = () => {
  ...
  const config =
    siteContent.sections.find((s) => s.id === "integrations")?.content || {};

  const title = config.title || "Connected Ecosystem";
  const description =
    config.description ||
    "We play well with the tools you already love. Direct integrations ensure your data flows where it needs to go.";
  return (
    <section
      id="integrations"
      className="py-24 bg-white overflow-hidden border-y border-slate-100 relative"
    >
      {/* Line grid overlay */}
      <div
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <div className="order-1">
            <span className="text-poble-gold font-black uppercase tracking-[0.3em] text-[10px] mb-6 block">
              Ecosystem
            </span>
            ...
            <div className="space-y-4 pt-6 border-t border-slate-100">
              ...
              <p className="text-sm font-extrabold text-poble-charcoal opacity-60 leading-relaxed">
                We are continuously partnering with local Australian platforms{" "}
                <br className="hidden md:block" />
                to ensure <span className="font-logo">poble</span> adapts to
                your specific venue needs.
              </p>
            </div>
          </div>
