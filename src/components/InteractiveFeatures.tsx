...
const FeatureCard: React.FC<FeatureCardProps> = ({ title, desc, icon: Icon }) => (
  <div className="p-6 border border-slate-100 bg-white rounded-2xl hover:border-slate-200 hover:shadow-sm transition-all duration-200">
    ...
    <h3 className="text-base font-bold text-poble-charcoal tracking-tight mb-2 font-heading">
      {title[0]}{title[1] ? <span className="text-slate-400"> {title[1]}</span> : null}
    </h3>
  ...
  const features = [
    {
      ...
      desc: config.feature1_collapsed || "Control menus and staff access with ease. Everything stays in sync across your POS.",
      icon: Cloud,
    },
    {
      title: getSplitTitle(config.feature2_title),
      desc: config.feature2_collapsed || "Built for peak-hour service. Orders move fast without slowing your team.",
      icon: Zap,
    },
    {
      title: getSplitTitle(config.feature3_title),
      desc: config.feature3_collapsed || "Native EFTPOS integration for Australian venues. No double handling.",
      icon: CreditCard,
    },
    {
      title: getSplitTitle(config.feature4_title),
      desc: config.feature4_collapsed || "Edit menus instantly. Updates apply right away.",
      icon: RefreshCcw,
    },
    {
      title: getSplitTitle(config.feature5_title),
      desc: config.feature5_collapsed || "Clear table layouts and kitchen display support. Keep service flowing.",
      icon: LayoutGrid,
    },
    {
      title: getSplitTitle(config.feature6_title),
      desc: config.feature6_collapsed || "Built-in loyalty and online ordering to support repeat customers.",
      icon: Users,
    },
  ];
