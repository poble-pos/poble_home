...
export const Testimonials: React.FC = () => {
    ...
    const reviews = [
        {
            id: 1,
            quote: config.quote1_text || "The fastest system we have ever used. Staff picked it up in minutes.",
        ...
        {
            id: 3,
            quote: config.quote3_text || "We use two POS terminals to handle entry ticketing and café orders simultaneously.",
            author: config.quote3_author || "Sarah Lee",
            role: config.quote3_venue || "Kidsday (Sydney)",
            image: "https://images.unsplash.com/photo-1589156280159-27698a70f29e?auto=format&fit=crop&q=80&w=100&h=100"
        }
    ];

    const title = config.title || "Loved by Aussie Venues";
    const description = config.description || "Join hundreds of cafes, bars and restaurants that have switched to poble for a faster, simpler service.";
    return (
        <section className="py-24 bg-slate-50 border-y border-slate-100 overflow-hidden relative">
            ...
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="text-center max-w-3xl mx-auto mb-16 md:mb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    <span className="text-poble-gold font-black uppercase tracking-[0.3em] text-xs mb-6 block">Trusted by Locals</span>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                    {reviews.map((review, i) => (
                        <div
                            key={review.id}
                            className={`bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:border-poble-gold/30 hover:-translate-y-1 transition-all duration-300 relative group animate-in fade-in slide-in-from-bottom-8 duration-700 ${i === 0 ? 'delay-0' : i === 1 ? 'delay-150' : 'delay-300'
                                }`}
                        >
                            ...
                            <div className="flex items-center gap-4 mt-auto">
                                ...
                                <div>
                                    <div className="font-black text-sm text-poble-charcoal tracking-tight">{review.author}</div>
                                    <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mt-0.5">{review.role}</div>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="mt-20 pt-10 border-t border-slate-200/60 flex flex-wrap justify-center gap-x-12 gap-y-8">
                    {[
                        ...
                        <div key={i} className="flex items-center gap-3">
                            ...
                            <div className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-relaxed whitespace-pre-line">{stat.label}</div>
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-24 relative overflow-hidden">
                    ...
                    <p className="text-center text-xs font-black uppercase tracking-[0.3em] text-slate-400 mb-8">Launch Partners</p>
                    ...
                    <div className="flex animate-logo-scroll gap-16 items-center w-max py-6">
                        ...
                        {[
                            ...
                            <div key={`logo-1-${i}`} className="h-16 w-32 md:w-40 relative opacity-80 hover:opacity-100 hover:scale-105 transition-all duration-500">
                                <img
                                    src={logo.src}
                                    alt={logo.name}
                        ...
                        {[
                            ...
                            <div key={`logo-2-${i}`} className="h-16 w-32 md:w-40 relative opacity-80 hover:opacity-100 hover:scale-105 transition-all duration-500">
                                <img
                                    src={logo.src}
                                    alt={logo.name}
                                    className="w-full h-full object-contain"
