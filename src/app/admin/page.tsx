...
export default function AdminDashboard() {
  ...
  const [selectedSectionId, setSelectedSectionId] = useState(
    siteContent.sections[0]?.id || null,
  );
  ...
  const selectedSection = siteContent.sections.find(
  ...
  const isModified =
    JSON.stringify(localContent) !== JSON.stringify(selectedSection?.content);
  const isLayoutModified =
    JSON.stringify(localSections) !== JSON.stringify(siteContent.sections);
  ...
  const handleUpdateLayout = async () => {
    ...
    showToast(
      "Layout order updated in memory. Click Publish to save live.",
      "info",
    );
  ...
  const handleUpdateSection = async () => {
    ...
    setShowSuccess(true);
    showToast(
      `Section "${selectedSection?.name}" updated. Click Publish to save live.`,
      "info",
    );
  ...
  return (
    <div className="min-h-screen bg-[#FDFCF9] flex font-sans text-poble-charcoal">
      {/* Sidebar */}
      <aside
        className={`bg-white border-r border-slate-100 flex flex-col h-screen sticky top-0 shrink-0 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? "w-20" : "w-72"}`}
      >
        <div
          className={`p-6 border-b border-slate-50 flex items-center ${isSidebarCollapsed ? "justify-center" : "justify-between"}`}
        >
          {!isSidebarCollapsed && (
            <div className="flex items-center gap-3 animate-in fade-in duration-300">
              <Logo className="h-8" />
              <span className="bg-poble-gold/10 text-poble-gold text-[9px] font-black uppercase px-2 py-0.5 rounded tracking-widest shrink-0">
                Admin
              </span>
            </div>
          )}
        ...
        <nav
          className={`flex-1 space-y-1 overflow-y-auto px-4 py-6 ${isSidebarCollapsed ? "flex flex-col items-center" : ""}`}
        >
          {!isSidebarCollapsed && (
              ...
              return (
                <div key={section.id} className="relative group/nav mb-1">
                  <button
                    ...
                    title={
                      isSidebarCollapsed
                        ? `${section.name} ${!section.visible ? "(Hidden)" : ""}`
                        : ""
                    }
                  >
                    <Icon
                      className={`shrink-0 transition-all ${isSidebarCollapsed ? "w-6 h-6" : "w-4 h-4 opacity-70"}`}
                    />
                    {!isSidebarCollapsed && (
                  ...
                  {!isSidebarCollapsed && (
                    <div
                      className={`absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 transition-all duration-300 ${isActive ? "opacity-100" : "opacity-0 group-hover/nav:opacity-100"}`}
                    >
                      <div className="flex flex-col">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            // Local Reorder Logic
                            const sorted = [...localSections].sort(
                              (a, b) => a.order - b.order,
                            );
                            if (idx > 0) {
          ...
          <div
            className={`mt-8 ${isSidebarCollapsed ? "w-full h-px bg-slate-50 mb-6" : ""}`}
          >
            {!isSidebarCollapsed && (
              <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 ml-4 animate-in fade-in">
                Settings
              </div>
            )}
            <button
            ...
            >
              <Settings
                className={`shrink-0 ${isSidebarCollapsed ? "w-6 h-6" : "w-4 h-4 text-slate-400"}`}
              />
        ...
        <div
          className={`p-4 border-t border-slate-50 mt-auto ${isSidebarCollapsed ? "flex justify-center" : ""}`}
        >
          ...
          >
            <LogOut
              className={`shrink-0 ${isSidebarCollapsed ? "w-6 h-6" : "w-4 h-4"}`}
            />
        ...
        <header className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-slate-600 font-bold text-sm tracking-tight">
              Modify live content without code deployments.
            </p>
          </div>
            ...
            {selectedSection ? (
              <div className="bg-white rounded-[2.5rem] border border-slate-100 p-8 md:p-12 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="flex items-center gap-4">
                    <div
                      className={`flex items-center gap-2 px-3 py-1 rounded-full border transition-all duration-500 ${isModified || showSuccess ? "bg-amber-50 text-amber-600 border-amber-100" : "bg-emerald-50 text-emerald-600 border-emerald-100"}`}
                    >
                      {showSuccess ? (
                        <>
                          <Check className="w-3 h-3" />
                          ...
                          typeof value === "string" ? (
                              ...
                              <div className="flex items-start gap-6">
                                ...
                                <div className="flex-1 space-y-4">
                                  <p className="text-[11px] font-bold text-slate-600 leading-tight">
                                    Recommended: PNG, WEBP or JPEG.
                                    <br />
                                    Max size: 2MB for display performance.
                                  </p>
                            ...
                            <div className="space-y-4">
                              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {(value as string[]).map((imgUrl, idx) => (
                                  <div
                                    key={idx}
                                    className="p-4 rounded-[1.8rem] bg-slate-50 border border-slate-100 space-y-3 group/img"
                                  >
                            ...
                            <div className="space-y-4">
                              {(value as any[]).map((item, idx) => (
                                <details
                                  key={idx}
                                  className="group/accordion bg-white border border-slate-200 rounded-2xl overflow-hidden [&[open]]:ring-2 [&[open]]:ring-poble-gold/10 [&[open]]:bg-slate-50/50 transition-all"
                                >
                                              ...
                                              <div className="grid grid-cols-1 gap-4">
                                                {fieldValue.map(
                                                  (subItem, subIdx) => (
                                                    <div
                                                      key={subIdx}
                                                      className="group relative flex gap-5 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-poble-gold/20 transition-all"
                                                    >
                                                      <div
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                onClick={async () => {
                  ...
                  const nextSections = isLayoutModified
                    ? localSections
                    : siteContent.sections;
                  ...
                  if (isModified && selectedSectionId)
                    updateSectionContent(selectedSectionId, localContent);
